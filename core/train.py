...
def train_VDSR(cfg, init_epoch, train_data_loader, val_data_loader, net, solver, lr_scheduler, 
    ckpt_dir, train_writer, val_writer, Best_PSNR, Best_Epoch, scaler):
    # Training loop
    for epoch_idx in range(init_epoch, cfg.TRAIN.NUM_EPOCHES):
        # Tick / tock
        epoch_start_time = time()
        ...
        for batch_idx, (_, img_lr_s, img_lr, img_hr) in enumerate(train_data_loader):
            ...
            # Gradient decent and gradient clipping
            scaler.scale(total_loss).backward()
            scaler.unscale_(solver)
            torch.nn.utils.clip_grad_norm_(net.parameters(), max_norm=0.01 / solver.param_groups[0]["lr"], norm_type=2.0)
